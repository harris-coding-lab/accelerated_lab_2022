---
title: "TA Session 12: Writing Functions"
...
- Break up your problem into bite sized pieces, try the components separately, and then put them together.  You'll see some of this philosophy in this lab. 
  ...
  - The "bite-sized pieces" should encapsulate some logical set of actions that make sense together. For example, tidyverse takes `subset()` and encapsulates row subsetting with `filter()` and column subsetting with `select()`. If they went any further,^[e.g. a function for evaluating the filter call and another for doing the row subsetting] the functions would be harder to use. 
