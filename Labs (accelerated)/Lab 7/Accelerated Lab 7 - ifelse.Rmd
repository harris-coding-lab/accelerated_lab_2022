---
title: "TA Session 4: If statements and conditionals"
...
1. The final thing is a concept. You're probably about to write code that looks like the following pseudo code.^[pseudo code is a term for fake code that captures the logic of some coding idea without being actual code.] Note that `mutate()` is a function that creates new variables based on certain conditions. This function is included in a package called tidyverse (you need to install and load it before running the code), though you don't have to worry about it now -- we will cover more details in Lecture 7.
    ...
    Test out the concept with a simple example. For example, try to figure out Abdul's only condition of being Ivy Plus. If time permits, also try to figure out Stephen's only condition of not being a publi school.
