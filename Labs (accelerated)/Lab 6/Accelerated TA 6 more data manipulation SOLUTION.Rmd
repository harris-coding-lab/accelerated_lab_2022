---
title: "Solutions Accelerated Coding Lab: More data manipulation"
author: "Ari Anisfeld"
date: "09-04-2022"
urlcolor: blue
output:
...
identical(
  ...
  txhousing[order(-txhousing$year, txhousing$month, -txhousing$sales),]
)
```
...
5. Which of the following code works? Can you think of two additional strategies to get filter the data so we only have data from IA, IL, MI and WI using `filter`?^[*Hint* one strategy we discussed in class; for the second strategy, look at the previous problem.]
...
txhousing %>%
  filter(month %in% c(12, 1, 2),
        between(year, 2013, 2016),
        volume > 1e9) %>%
  select(city, year, month, volume, listings)
```
...
# Solution
txhousing %>%
  select(city, year, month, volume, sales, median) %>%
  mutate(mean_sales_price = round(volume / sales),
         best_month_sales_price = max(mean_sales_price, na.rm=TRUE),
         percent_of_best = mean_sales_price / best_month_sales_price)
```
...
# Better style
temp <- txhousing[order(-txhousing$volume),]
temp <- within(temp, {
               mean_sales_price <- round(volume / sales)
               diff_mean_median <- abs(mean_sales_price - median)
               })
temp <- subset(temp,
      city == "Houston" & 
      (year == 2014  | year == 2015 | year == 2013),
      select = c(city, year, month, mean_sales_price, diff_mean_median, volume))
head(temp)
...
txhousing %>%
  ...
  mutate(mean_sales_price = round(volume / sales),
         diff_mean_median = abs(mean_sales_price - median)) %>%
  filter(city == "Houston", (year == 2014  | year == 2015 | year == 2013)) %>%
  select(c(city, year, month, mean_sales_price,  diff_mean_median, volume))
```
